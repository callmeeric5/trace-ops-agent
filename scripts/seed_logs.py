...
def _make_logs() -> list[LogEntryORM]:
    ...
    # Normal operation logs (~200)
    for _ in range(200):
        service = random.choice(list(services_info.keys()))
        template, level = random.choice(services_info[service])
        ts = now - timedelta(minutes=random.randint(1, 59), seconds=random.randint(0, 59))
        msg = template.format(
    ...
    # ---- Bug 1: DB connection pool leak (order-service) ----
    trace_id = str(uuid4())
    base_time = now - timedelta(minutes=15)
    logs.extend([
        LogEntryORM(
            id=str(uuid4()), timestamp=base_time,
            service="order-service", level=LogLevel.WARNING,
            message="Connection pool utilization at 85% (170/200 connections in use)",
            trace_id=trace_id,
            metadata_json='{"pool_size": 200, "active": 170}',
        ),
        LogEntryORM(
            id=str(uuid4()), timestamp=base_time + timedelta(seconds=30),
            service="order-service", level=LogLevel.ERROR,
        ...
        LogEntryORM(
            id=str(uuid4()), timestamp=base_time + timedelta(seconds=60),
            service="order-service", level=LogLevel.CRITICAL,
    ...
    # ---- Bug 2: Cache penetration (inventory-service) ----
    trace_id2 = str(uuid4())
    base_time2 = now - timedelta(minutes=10)
    logs.extend([
        LogEntryORM(
            id=str(uuid4()), timestamp=base_time2,
            service="inventory-service", level=LogLevel.WARNING,
        ...
        LogEntryORM(
            id=str(uuid4()), timestamp=base_time2 + timedelta(seconds=20),
            service="inventory-service", level=LogLevel.ERROR,
        ...
        LogEntryORM(
            id=str(uuid4()), timestamp=base_time2 + timedelta(seconds=40),
            service="inventory-service", level=LogLevel.CRITICAL,
            message="Inventory service returning stale data. Fallback cache activated. Orders may accept out-of-stock items.",
            trace_id=trace_id2,
        ),
    ])

    # ---- Bug 3: Duplicate charge (payment-service) ----
    trace_id3 = str(uuid4())
    base_time3 = now - timedelta(minutes=5)
    logs.extend([
        LogEntryORM(
            id=str(uuid4()), timestamp=base_time3,
            service="payment-service", level=LogLevel.WARNING,
            message="Stripe gateway response time elevated: 4.8s (SLA: 2s)",
            trace_id=trace_id3,
            metadata_json='{"gateway": "stripe", "response_time_ms": 4800}',
        ),
        LogEntryORM(
            id=str(uuid4()), timestamp=base_time3 + timedelta(seconds=15),
            service="payment-service", level=LogLevel.ERROR,
        ...
        LogEntryORM(
            id=str(uuid4()), timestamp=base_time3 + timedelta(seconds=30),
            service="payment-service", level=LogLevel.CRITICAL,
